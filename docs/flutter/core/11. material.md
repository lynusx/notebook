# material -- Material Design组件库

实现 Google 的 **Material Design** 设计规范的完整组件集合，包括主题、导航、输入、反馈、布局等，是 Flutter 最常用的 UI 库。

### 1.1 核心能力

- Material 3 主题系统（ThemeData、ColorScheme、Typography、ShapeTheme）
- 应用结构（Scaffold、AppBar、BottomNavigationBar、NavigationBar、NavigationRail、Drawer）
- 按钮与控件（ElevatedButton、TextButton、OutlinedButton、FloatingActionButton、IconButton、SegmentedButton、Switch、Checkbox、Radio、Slider）
- 输入组件（TextField、TextFormField、Autocomplete、DropdownButton、DatePicker、TimePicker、SearchAnchor）
- 列表与卡片（ListTile、Card、DataTable、PaginatedDataTable、Stepper、ExpansionPanel）
- 反馈组件（SnackBar、AlertDialog、SimpleDialog、BottomSheet、ModalBottomSheet、Tooltip）
- 进度与指示器（CircularProgressIndicator、LinearProgressIndicator、RefreshIndicator）
- 选择器与 Chips（ChoiceChip、FilterChip、InputChip、ActionChip）
- 导航组件（TabBar、BottomNavigationBar、NavigationDrawer、PageTransitionsTheme）
- Material 特定动画（InkWell、InkResponse、MaterialPageRoute、MaterialRectCenterArcTween）
...
│   ├── ThemeData                 # 主题数据
│   ├── Theme                     # 主题继承 Widget
│   ├── ColorScheme               # 颜色方案 (Material 3)
│   ├── ColorScheme.fromSeed()    # 从种子色生成
│   ├── ColorScheme.fromImageProvider()  # 从图像生成
│   ├── MaterialColor             # Material 主色
│   ├── MaterialAccentColor       # Material 强调色
│   ├── MaterialBasedCupertinoThemeData  # 基于 Material 的 Cupertino 主题
│   ├── Typography                # 字体排版
│   ├── TextTheme                 # 文本主题
│   ├── VisualDensity             # 视觉密度
│   ├── ThemeMode                 # 主题模式 (light/dark/system)
│   ├── MaterialStateProperty<T>  # Material 状态属性
│   ├── MaterialStateColor        # Material 状态颜色
│   ├── MaterialStateOutlinedBorder    # Material 状态边框
│   ├── MaterialStateBorderSide   # Material 状态边框边
│   ├── MaterialStateMouseCursor  # Material 状态鼠标指针
│   ├── MaterialStateTextStyle    # Material 状态文本样式
│   ├── MaterialStatePropertyAll<T>    # 全状态属性
│   ├── MaterialStatesController  # 状态控制器
│   ├── NoDefaultCupertinoThemeData    # 无默认 Cupertino 主题
│   ├── ButtonThemeData           # 按钮主题 (已弃用)
│   ├── CardTheme                 # 卡片主题
│   ├── CardThemeData             # 卡片主题数据
│   ├── ChipThemeData             # Chip 主题
│   ├── InputDecorationTheme      # 输入装饰主题
│   ├── SliderThemeData           # 滑块主题
│   ├── TabBarTheme               # 标签栏主题
│   ├── TabBarThemeData           # 标签栏主题数据
│   ├── BottomNavigationBarTheme  # 底部导航栏主题
│   ├── BottomNavigationBarThemeData   # 底部导航栏主题数据
│   ├── NavigationBarTheme        # 导航栏主题 (Material 3)
│   ├── NavigationBarThemeData    # 导航栏主题数据
│   ├── NavigationRailTheme       # 导航轨道主题
│   ├── NavigationRailThemeData   # 导航轨道主题数据
│   ├── DrawerThemeData           # 抽屉主题
│   ├── FloatingActionButtonThemeData  # FAB 主题
│   ├── IconButtonThemeData       # 图标按钮主题
│   ├── PopupMenuThemeData        # 弹出菜单主题
│   ├── DialogTheme               # 对话框主题
│   ├── DialogThemeData           # 对话框主题数据
│   ├── SnackBarThemeData         # SnackBar 主题
│   ├── TooltipThemeData          # 工具提示主题
│   ├── MaterialBannerThemeData   # 横幅主题
│   ├── DividerThemeData          # 分隔线主题
│   ├── ListTileThemeData         # 列表项主题
│   ├── CheckboxThemeData         # 复选框主题
│   ├── RadioThemeData            # 单选框主题
│   ├── SwitchThemeData           # 开关主题
│   ├── ProgressIndicatorThemeData     # 进度指示器主题
│   ├── AppBarTheme               # 应用栏主题
│   ├── ElevatedButtonThemeData   # 凸起按钮主题
│   ├── TextButtonThemeData       # 文本按钮主题
│   ├── OutlinedButtonThemeData   # 轮廓按钮主题
│   ├── SegmentedButtonThemeData  # 分段按钮主题
│   ├── FilledButtonThemeData     # 填充按钮主题
│   ├── MenuThemeData             # 菜单主题
│   ├── DropdownMenuThemeData     # 下拉菜单主题
│   ├── TimePickerThemeData       # 时间选择器主题
│   ├── DatePickerThemeData       # 日期选择器主题
│   ├── ToggleButtonsThemeData    # 切换按钮主题
│   ├── ActionIconThemeData       # 操作图标主题
│   ├── BadgeThemeData            # 徽章主题
│   ├── BottomAppBarTheme         # 底部应用栏主题
│   ├── BottomSheetThemeData      # 底部表单主题
│   ├── DataTableThemeData        # 数据表主题
│   ├── ExpansionTileThemeData    # 展开列表项主题
│   ├── FilledButtonTheme         # 填充按钮主题
│   ├── IconButtonTheme           # 图标按钮主题
│   ├── ListTileTheme             # 列表项主题
│   ├── MenuBarThemeData          # 菜单栏主题
│   ├── MenuButtonThemeData       # 菜单按钮主题
│   ├── NavigationDrawerThemeData # 导航抽屉主题
│   ├── PopupMenuTheme            # 弹出菜单主题
│   ├── ProgressIndicatorTheme    # 进度指示器主题
│   ├── RadioTheme                # 单选框主题
│   ├── SearchBarThemeData        # 搜索栏主题
│   ├── SearchViewThemeData       # 搜索视图主题
│   ├── SegmentedButtonTheme      # 分段按钮主题
│   ├── SliderTheme               # 滑块主题
...
│   ├── MaterialApp.router        # 路由器构造
│   ├── Scaffold                  # 脚手架
│   ├── ScaffoldMessenger         # 脚手架信使
│   ├── ScaffoldMessengerState    # 脚手架信使状态
│   ├── ScaffoldPrelayoutGeometry # 脚手架预布局几何
│   ├── ScaffoldState             # 脚手架状态
│   ├── AppBar                    # 应用栏
│   ├── SliverAppBar              # 可折叠应用栏
│   ├── AppBarThemeData           # 应用栏主题数据
│   ├── BottomAppBar              # 底部应用栏
│   ├── BottomAppBarThemeData     # 底部应用栏主题数据
│   ├── NavigationBar             # 导航栏 (Material 3)
│   ├── NavigationDestination     # 导航目的地
│   ├── NavigationIndicator       # 导航指示器
│   ├── NavigationRail            # 导航轨道
│   ├── NavigationRailDestination # 轨道目的地
│   ├── NavigationDrawer          # 导航抽屉 (Material 3)
│   ├── NavigationDrawerDestination  # 抽屉目的地
│   ├── Drawer                    # 抽屉
│   ├── DrawerController          # 抽屉控制器
│   ├── DrawerAlignment           # 抽屉对齐
│   ├── BottomNavigationBar       # 底部导航栏 (旧版)
│   ├── BottomNavigationBarItem   # 底部导航项
│   ├── BottomNavigationBarType   # 底部导航栏类型
│   ├── TabBar                    # 标签栏
│   ├── TabBarView                # 标签栏视图
│   ├── Tab                       # 标签
│   ├── TabController             # 标签控制器
│   ├── DefaultTabController      # 默认标签控制器
│   ├── TabPageSelector           # 标签页选择器
│   ├── TabPageSelectorIndicator  # 标签页选择器指示器
│   ├── FloatingActionButton      # 浮动操作按钮
│   ├── FloatingActionButton.extended()  # 扩展 FAB
│   ├── FloatingActionButton.small()     # 小 FAB
│   ├── FloatingActionButton.large()     # 大 FAB
│   ├── FloatingActionButtonLocation     # FAB 位置
│   ├── FloatingActionButtonAnimator     # FAB 动画器
│   └── RouteObserver             # 路由观察者
│
├── 按钮
│   ├── ElevatedButton            # 凸起按钮
│   ├── ElevatedButton.icon()     # 带图标凸起按钮
│   ├── FilledButton              # 填充按钮
│   ├── FilledButton.icon()       # 带图标填充按钮
│   ├── FilledButton.tonal()      # 色调填充按钮
│   ├── FilledButton.tonalIcon()  # 带图标色调填充按钮
│   ├── OutlinedButton            # 轮廓按钮
│   ├── OutlinedButton.icon()     # 带图标轮廓按钮
│   ├── TextButton                # 文本按钮
│   ├── TextButton.icon()         # 带图标文本按钮
│   ├── IconButton                # 图标按钮
│   ├── IconButton.filled()       # 填充图标按钮
│   ├── IconButton.filledTonal()  # 色调填充图标按钮
│   ├── IconButton.outlined()     # 轮廓图标按钮
│   ├── IconButton.styleFrom()    # 样式工厂
│   ├── FloatingActionButton      # 浮动操作按钮 (已在应用结构)
│   ├── SegmentedButton<T>        # 分段按钮
│   ├── ButtonSegment<T>          # 按钮段
│   ├── ButtonStyle               # 按钮样式
│   ├── ButtonStyleButton         # 样式按钮基类
│   ├── TextButton.styleFrom()    # 文本按钮样式工厂
│   ├── ElevatedButton.styleFrom()     # 凸起按钮样式工厂
│   ├── OutlinedButton.styleFrom()     # 轮廓按钮样式工厂
│   ├── FilledButton.styleFrom()       # 填充按钮样式工厂
│   ├── SegmentedButton.styleFrom()    # 分段按钮样式工厂
│   └── MaterialButton            # 旧版 Material 按钮 (已弃用)
│
├── 选择控件
│   ├── Checkbox                  # 复选框
│   ├── Checkbox.adaptive()       # 自适应平台样式
│   ├── CheckboxListTile          # 复选框列表项
│   ├── CheckboxTheme             # 复选框主题
│   ├── Radio<T>                  # 单选框
│   ├── Radio.adaptive()          # 自适应平台样式
│   ├── RadioListTile<T>          # 单选框列表项
│   ├── RadioTheme                # 单选框主题
│   ├── Switch                    # 开关
│   ├── Switch.adaptive()         # 自适应平台样式
│   ├── SwitchListTile            # 开关列表项
│   ├── SwitchTheme               # 开关主题
│   ├── Slider                    # 滑块
│   ├── RangeSlider               # 范围滑块
│   ├── RangeValues               # 范围值
│   ├── SliderComponentShape      # 滑块组件形状
│   ├── SliderTickMarkShape       # 刻度标记形状
│   ├── SliderTrackShape          # 滑块轨道形状
│   ├── SliderThumbShape          # 滑块 thumb 形状
│   ├── SliderValueIndicatorShape # 值指示器形状
│   ├── RangeSliderThumbShape     # 范围滑块 thumb 形状
│   ├── RangeSliderValueIndicatorShape  # 范围滑块值指示器形状
│   ├── RangeSliderTrackShape     # 范围滑块轨道形状
│   ├── RangeSliderTickMarkShape  # 范围滑块刻度标记形状
│   ├── YearPicker                # 年份选择器
│   ├── MonthPicker               # 月份选择器
│   ├── DayPicker                 # 日期选择器
│   ├── ToggleButtons             # 切换按钮组 (已弃用)
│   ├── ToggleButtonsBuilder      # 切换按钮构建器
│   └── ToggleButtonsState        # 切换按钮状态
│
├── 输入组件
│   ├── TextField                 # 文本字段
│   ├── TextFormField             # 文本表单字段
│   ├── InputDecoration           # 输入装饰
│   ├── InputDecoration.collapsed()    # 折叠输入装饰
│   ├── OutlineInputBorder        # 轮廓输入边框
│   ├── UnderlineInputBorder      # 下划线输入边框
│   ├── InputBorder               # 输入边框基类
│   ├── InputCounterWidgetBuilder # 计数器构建器
│   ├── TextSelectionControls     # 文本选择控制
│   ├── MaterialTextSelectionControls  # Material 文本选择
│   ├── DropdownButton<T>         # 下拉按钮
│   ├── DropdownButtonFormField<T>     # 下拉表单字段
│   ├── DropdownButtonHideUnderline    # 隐藏下划线
│   ├── DropdownMenu<T>           # 下拉菜单 (Material 3)
│   ├── DropdownMenuEntry<T>      # 菜单条目
│   ├── DropdownMenuItem<T>       # 下拉菜单项
│   ├── Autocomplete<T>           # 自动完成
│   ├── RawAutocomplete<T>        # 原始自动完成
│   ├── AutocompleteHighlightedOption  # 高亮选项
│   ├── AutocompletePreviousOptionIntent  # 上一个意图
│   ├── AutocompleteNextOptionIntent      # 下一个意图
│   ├── SearchAnchor              # 搜索锚点
│   ├── SearchAnchor.bar()        # 搜索栏
│   ├── SearchBar                 # 搜索栏
│   ├── SearchViewTheme           # 搜索视图主题
│   ├── DatePicker                # 日期选择器
│   ├── showDatePicker()          # 显示日期选择器
│   ├── DateRangePickerDialog     # 日期范围选择器对话框
│   ├── showDateRangePicker()     # 显示日期范围选择器
│   ├── DatePickerDialog          # 日期选择器对话框
│   ├── TimePicker                # 时间选择器
│   ├── showTimePicker()          # 显示时间选择器
│   ├── TimePickerDialog          # 时间选择器对话框
│   ├── CalendarDatePicker        # 日历日期选择器
│   ├── InputDatePickerFormField  # 输入日期表单字段
│   ├── DatePickerEntryMode       # 进入模式枚举
│   ├── DatePickerMode            # 模式枚举
│   ├── SelectableDate            # 可选择日期
│   ├── RestorableDatePickerEntryMode  # 可恢复日期模式
│   ├── RestorableTimePickerEntryMode  # 可恢复时间模式
│   ├── TimeOfDay                 # 时间
│   ├── TimeOfDayFormat           # 时间格式
│   ├── HourFormat                # 小时格式
│   ├── DayPeriod                 # 时段 (上午/下午)
│   └── DateUtils                 # 日期工具
│
...
│   ├── Chip                      # 基础 Chip
│   ├── ChipTheme                 # Chip 主题
│   ├── ChipAttributes            # Chip 属性
│   ├── DeletableChipAttributes   # 可删除 Chip 属性
│   ├── SelectableChipAttributes  # 可选 Chip 属性
│   ├── CheckmarkableChipAttributes    # 可标记 Chip 属性
│   ├── TappableChipAttributes    # 可点击 Chip 属性
│   ├── DisabledChipAttributes    # 禁用 Chip 属性
│   ├── InputChip                 # 输入 Chip
│   ├── ChoiceChip<T>             # 选择 Chip
│   ├── FilterChip<T>             # 过滤 Chip
│   ├── FilterChip.elevated()     # 凸起过滤 Chip
│   ├── ActionChip                # 操作 Chip
│   ├── ActionChip.elevated()     # 凸起操作 Chip
│   └── RawChip                   # 原始 Chip
│
├── 列表与布局
│   ├── ListTile                  # 列表项
│   ├── ListTile.divideTiles()    # 分隔列表项
│   ├── ListTileTitleAlignment    # 标题对齐
│   ├── CheckboxListTile          # 复选框列表项
│   ├── RadioListTile             # 单选框列表项
│   ├── SwitchListTile            # 开关列表项
│   ├── ExpansionTile             # 展开列表项
│   ├── ExpansionTileController   # 展开列表项控制器
│   ├── ExpansionPanel            # 扩展面板
│   ├── ExpansionPanelList        # 扩展面板列表
│   ├── ExpansionPanelRadio       # 单选扩展面板
│   ├── ExpansionPanelHeaderBuilder    # 头部构建器
│   ├── Card                      # 卡片
│   ├── Card.filled()             # 填充卡片
│   ├── Card.outlined()           # 轮廓卡片
│   ├── CardTheme                 # 卡片主题
...
│   ├── DataTable                 # 数据表格
│   ├── DataColumn                # 数据列
│   ├── DataColumnSortCallback    # 列排序回调
│   ├── DataRow                   # 数据行
│   ├── DataCell                  # 数据单元格
│   ├── DataTableSortArrowDirection    # 排序箭头方向
│   ├── PaginatedDataTable        # 分页数据表格
│   ├── PaginatedDataTableState   # 分页数据表格状态
│   ├── TableRowInkWell           # 表格行水波纹
│   ├── Stepper                   # 步骤器
│   ├── StepperType               # 步骤器类型
│   ├── Step                      # 步骤
│   ├── StepState                 # 步骤状态
│   ├── MaterialBanner            # Material 横幅
│   ├── MaterialBannerClosedReason     # 关闭原因
│   ├── AboutDialog               # 关于对话框
│   ├── AboutListTile             # 关于列表项
│   ├── LicensePage               # 许可证页面
...
│   ├── RefreshIndicator.adaptive()    # 自适应下拉刷新
│   ├── RefreshIndicatorTriggerMode    # 触发模式
│   ├── RefreshIndicatorState     # 刷新状态
│   └── ReorderableListView       # 可重排序列表
│
├── 反馈与提示
│   ├── SnackBar                  # 底部提示条
│   ├── SnackBarAction            # SnackBar 操作
│   ├── SnackBarBehavior          # 行为枚举
│   ├── SnackBarClosedReason      # 关闭原因枚举
│   ├── SnackBarTheme             # SnackBar 主题
│   ├── Tooltip                   # 工具提示
│   ├── TooltipVisibility         # 工具提示可见性
│   ├── TooltipTriggerMode        # 触发模式枚举
│   ├── TooltipTheme              # 工具提示主题
│   ├── AlertDialog               # 警告对话框
│   ├── AlertDialog.icon()        # 带图标警告对话框
│   ├── SimpleDialog              # 简单对话框
│   ├── SimpleDialogOption        # 简单对话框选项
│   ├── Dialog                    # 通用对话框
│   ├── Dialog.fullscreen()       # 全屏对话框
│   ├── showDialog()              # 显示对话框
│   ├── showAdaptiveDialog()      # 显示自适应对话框
│   ├── showGeneralDialog()       # 显示通用对话框
│   ├── DialogTheme               # 对话框主题
│   ├── BottomSheet               # 底部表单
│   ├── BottomSheetThemeData      # 底部表单主题数据
│   ├── showBottomSheet()         # 显示底部表单
│   ├── showModalBottomSheet()    # 显示模态底部表单
│   ├── showAdaptiveActionSheet() # 显示自适应操作表单
│   ├── ModalBottomSheetRoute     # 模态底部表单路由
│   ├── PersistentBottomSheetController  # 持久底部表单控制器
│   ├── MaterialBanner            # Material 横幅 (已在列表与布局)
│   ├── showMaterialBanner()      # 显示横幅
│   ├── ScaffoldFeatureController # 脚手架特性控制器
│   ├── MaterialBannerClosedReason     # 横幅关闭原因
│   ├── Badge                     # 徽章 (Material 3)
│   ├── BadgeTheme                # 徽章主题
│   └── Banner                    # 横幅 (角标)
│
├── 导航与路由
│   ├── MaterialPageRoute<T>      # Material 页面路由
│   ├── MaterialPage<T>           # Material 页面 (Navigator 2.0)
│   ├── PageTransitionsTheme      # 页面过渡主题
│   ├── ZoomPageTransitionsBuilder     # 缩放过渡构建器
│   ├── PredictiveBackPageTransitionsBuilder  # 预测返回过渡
│   ├── OpenUpwardsPageTransitionsBuilder     # 向上打开过渡
│   ├── FadeUpwardsPageTransitionsBuilder     # 向上淡入过渡
│   └── ModalRoute<T>             # 模态路由
│
├── 手势与交互
│   ├── InkWell                   # 水波纹点击效果
...
│   ├── InkDecoration             # 水波纹装饰
│   ├── InkFeature                # 墨水特性
│   ├── InteractiveInkFeature     # 交互式墨水特性
│   ├── InteractiveInkFeatureFactory    # 交互式墨水特性工厂
│   ├── Feedback                  # 触觉/声音反馈
│   ├── Feedback.forTap()         # 点击反馈
│   ├── Feedback.forLongPress()   # 长按反馈
│   └── MergeableMaterial         # 可合并 Material
│
├── 形状与边框
│   ├── ShapeBorder               # 形状边框基类
...
│   ├── CircleBorder              # 圆形边框
│   ├── ContinuousRectangleBorder # 连续圆角矩形
│   ├── StarBorder                # 星形边框
│   ├── StarBorder.polygon()      # 多边形星形
│   ├── LinearBorder              # 线性边框
│   ├── LinearBorderEdge          # 线性边框边
│   ├── BorderRadiusTween         # 圆角半径补间
│   ├── MaterialStateOutlineInputBorder  # Material 状态轮廓输入边框
│   └── MaterialStateUnderlineInputBorder # Material 状态下划线输入边框
│
├── 图标与字体
...
│   ├── IconButton                # 图标按钮 (已在按钮)
│   ├── Typography                # 字体排版
│   ├── TextTheme                 # 文本主题
│   ├── ThemeData                 # 主题数据 (已在主题)
│   ├── ColorScheme               # 颜色方案 (已在主题)
│   └── Colors                    # 颜色常量
│
└── 其他组件
    ├── Material                  # Material 设计纸片效果
    ├── MaterialType              # 材料类型枚举
    ├── AnimatedIcons             # 动画图标
    ├── AnimatedIcon              # 动画图标 Widget
    ├── AnimatedIconData          # 动画图标数据
    ├── ReorderableListView       # 可重排序列表 (已在列表与布局)
    ├── ReorderableDragStartListener    # 拖动监听
    ├── ReorderableDelayedDragStartListener  # 延迟拖动
    ├── MergeableMaterialItem     # 合并项
    ├── MaterialSlice             # Material 切片
    ├── MaterialGap               # Material 间隙
    ├── UserAccountsDrawerHeader  # 用户账户抽屉头部
    ├── AboutListTile             # 关于列表项 (已在列表与布局)
    ├── LicensePage               # 许可证页面 (已在列表与布局)
    ├── CircleAvatar              # 圆形头像
    ├── Chip                      # Chip (已在 Chips)
    ├── Divider                   # 分隔线 (已在列表与布局)
    ├── DrawerHeader              # 抽屉头部
    ├── ListTile                  # 列表项 (已在列表与布局)
    ├── PopupMenuButton           # 弹出菜单按钮
    ├── PopupMenuDivider          # 弹出菜单分隔线
    ├── PopupMenuEntry            # 弹出菜单条目
    ├── PopupMenuItem             # 弹出菜单项
    ├── CheckedPopupMenuItem      # 选中弹出菜单项
    ├── FlexibleSpaceBar          # 灵活空间栏
    ├── FlexibleSpaceBarSettings  # 灵活空间栏设置
    ├── MaterialStateMouseCursor  # Material 状态鼠标指针 (已在主题)
    ├── NavigationIndicator       # 导航指示器 (已在应用结构)
    └── ScaffoldFeatureController # 脚手架特性控制器 (已在反馈与提示)
```
