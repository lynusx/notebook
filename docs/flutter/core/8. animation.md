...
提供完整的动画支持，包括补间动画（Tween）、动画控制器（AnimationController）、曲线（Curve）和动画状态管理。

### 1.1 核心能力

- 动画抽象（Animation<T>)
- 补间动画（Tween、ColorTween、SizeTween、RectTween、IntTween、StepTween、ConstantTween）
- 曲线与插值（Curve、Curves、Interval、SawTooth、ElasticInOutCurve 等）
- 动画状态监听（AnimationStatusListener、AnimationStatus：dismissed、forward、reverse、completed）
- 动画组合（AnimatedBuilder、AnimatedWidget、ProxyAnimation、ReverseAnimation、CurvedAnimation）
- 评估函数（Animatable.evaluate、Animation.drive）
...
├── 动画核心抽象
│   ├── Animation<T>                    # 动画基类
│   ├── AnimationLocalListenersMixin    # 本地监听器混入
│   ├── AnimationLazyListenerMixin      # 懒监听器混入
│   ├── AnimationEagerListenerMixin     # 急切监听器混入
│   ├── AnimationWithParentMixin<T>     # 带父动画混入
│   ├── AnimationController             # 动画控制器
│   ├── AnimationBehavior               # 动画行为枚举
│   └── AnimationDirection              # 动画方向枚举
│
├── 动画状态与通知
│   ├── AnimationStatus                 # 动画状态枚举
│   ├── AnimationStatusListener         # 状态监听器回调类型
│   ├── AnimationNotification           # 动画通知
│   ├── AnimationController.addStatusListener()   # 添加状态监听
│   └── AnimationController.removeStatusListener()# 移除状态监听
│
├── 补间与可动画
│   ├── Animatable<T>                   # 可动画基类
│   ├── Tween<T>                        # 补间基类
...
│   ├── IntTween                        # 整数补间
│   ├── StepTween                       # 步进补间
│   ├── ConstantTween<T>                # 常量补间
│   ├── CurveTween                      # 曲线补间
│   ├── TweenSequence<T>                # 补间序列
│   ├── TweenSequenceItem<T>            # 补间序列项
│   ├── FlippedTweenSequence<T>         # 翻转补间序列
│   ├── Tween.begin                     # 补间起始值
│   ├── Tween.end                       # 补间结束值
│   ├── Tween.transform()               # 补间转换
│   └── Tween.lerp()                    # 线性插值
│
├── 曲线
│   ├── Curve                           # 曲线基类
...
│   ├── Interval                        # 区间曲线
│   ├── Linear                          # 线性曲线
│   ├── DecelerateCurve                 # 减速曲线
│   ├── ElasticInCurve                  # 弹性进入曲线
│   ├── ElasticOutCurve                 # 弹性退出曲线
│   ├── ElasticInOutCurve               # 弹性进入退出曲线
│   ├── BounceInCurve                   # 弹跳进入曲线
│   ├── BounceOutCurve                  # 弹跳退出曲线
│   ├── BounceInOutCurve                # 弹跳进入退出曲线
│   ├── SawTooth                        # 锯齿波曲线
│   ├── ThreePointCubic                 # 三点三次贝塞尔
│   ├── SlowMiddle                      # 中间慢曲线
│   ├── FlippedCurve                    # 翻转曲线
│   ├── CurvedAnimation.curve           # 前向曲线
│   └── CurvedAnimation.reverseCurve    # 反向曲线
│
├── 动画组合与修饰
│   ├── ProxyAnimation                  # 代理动画
│   ├── ReverseAnimation                # 反向动画
│   ├── CurvedAnimation                 # 曲线动画
│   ├── TrainHoppingAnimation           # 列车跳跃动画
│   ├── CompoundAnimation<T>            # 复合动画
│   ├── AnimationMean                   # 平均值动画
│   ├── AnimationMax                    # 最大值动画
│   ├── AnimationMin                    # 最小值动画
│   └── Listenable.merge()              # 监听器合并
│
├── 动画评估与驱动
│   ├── Animation.drive()               # 驱动方法
│   ├── Animatable.evaluate()           # 评估方法
│   ├── AnimationController.forward()   # 正向播放
│   ├── AnimationController.reverse()   # 反向播放
│   ├── AnimationController.animateTo() # 动画到指定值
│   ├── AnimationController.animateWith() # 使用 Simulation
│   ├── AnimationController.fling()     # 抛掷动画
│   ├── AnimationController.repeat()    # 重复播放
│   ├── AnimationController.stop()      # 停止动画
│   ├── AnimationController.reset()     # 重置动画
│   ├── AnimationController.dispose()   # 释放资源
│   ├── AnimationController.value       # 当前值
│   ├── AnimationController.duration    # 持续时间
│   ├── AnimationController.reverseDuration  # 反向持续时间
│   └── AnimationController.velocity    # 当前速度
│
└── 特殊动画类型
    ├── AlwaysStoppedAnimation<T>       # 恒定值动画
    ├── AlwaysCompleteAnimation         # 始终完成动画
    ├── AlwaysDismissedAnimation        # 始终取消动画
    └── AnimationController.unbounded   # 无界控制器
```
