...
提供整个 Flutter 框架依赖的最基础类、工具函数和常量，包括 ChangeNotifier、Bindings、Key 等核心抽象。

### 1.1 核心能力

- 基础抽象类（ChangeNotifier、ValueNotifier、Diagnosticable）
- 组件标识（Key、LocalKey、GlobalKey、ValueKey、UniqueKey）
- 调试与诊断（debugPrint、DiagnosticsNode、DiagnosticableTree）
- 绑定机制（BindingBase、WidgetsBinding、SchedulerBinding）
- 异步工具（SynchronousFuture、microtask、Timeline）
- 不可变集合（Listenable、ValueListenable）
- 平台适配（TargetPlatform、defaultTargetPlatform）
- 位掩码与枚举（Enum、BitMask、immutable、mustCallSuper 等元数据注解）
...
│   ├── Key                     # 组件标识基类
│   ├── LocalKey                # 局部键基类
│   ├── ValueKey<T>             # 值键 (基于值的相等性)
│   ├── UniqueKey               # 唯一键 (每次创建都不同)
│   ├── GlobalKey<T extends State<StatefulWidget>>  # 全局键
│   ├── GlobalObjectKey<T>      # 基于对象身份的全局键
│   ├── LabeledGlobalKey<T>     # 带调试标签的全局键
│   └── PageStorageKey<T>       # 页面存储键 (保持滚动位置)
│
├── 通知与监听
│   ├── Listenable              # 可监听对象基类
│   ├── ValueListenable<T>      # 值可监听接口
│   ├── ChangeNotifier          # 变更通知器实现
│   ├── ValueNotifier<T>        # 值变更通知器
│   ├── Listenable.merge()      # 合并多个可监听对象
│   ├── Notification            # 通知基类
│   ├── NotificationListener<T extends Notification>  # 通知监听器 Widget
│   ├── Notification.dispatch() # 分发通知
│   └── ScrollNotification       # 滚动通知 (实际在 widgets 库)
│
├── 绑定系统
│   ├── BindingBase             # 绑定基类 (单例模式)
│   ├── WidgetsBindingObserver  # Widgets 绑定观察者接口
│   ├── ServicesBinding         # 服务绑定混入
│   ├── SchedulerBinding        # 调度绑定混入 (帧调度)
│   ├── PaintingBinding         # 绘制绑定混入 (图片缓存)
│   ├── RendererBinding         # 渲染绑定混入
│   ├── WidgetsBinding          # Widgets 绑定混入
│   ├── GestureBinding          # 手势绑定混入
│   ├── SemanticsBinding        # 语义绑定混入
│   └── TestWidgetsFlutterBinding  # 测试绑定 (在 flutter_test)
│
├── 调试与诊断
│   ├── debugPrint()            # 调试打印 (带节流)
│   ├── debugPrintSynchronously()  # 同步调试打印
│   ├── debugPrintStack()       # 打印调用堆栈
│   ├── debugFillProperties()   # 填充诊断属性
│   ├── DiagnosticsNode         # 诊断节点基类
│   ├── Diagnosticable          # 可诊断接口
│   ├── DiagnosticableTree      # 可诊断树接口
│   ├── DiagnosticableTreeMixin # 可诊断树混入
│   ├── DiagnosticsProperty<T>  # 通用诊断属性
│   ├── StringProperty          # 字符串诊断属性
│   ├── DoubleProperty          # 双精度诊断属性
│   ├── IntProperty             # 整数诊断属性
│   ├── BoolProperty            # 布尔诊断属性
│   ├── EnumProperty<T>         # 枚举诊断属性
│   ├── IterableProperty<T>     # 可迭代诊断属性
│   ├── FlagProperty            # 标记诊断属性
│   ├── ObjectFlagProperty<T>   # 对象标记属性
│   ├── FlagsSummary<T>         # 标记摘要
│   ├── PercentProperty         # 百分比属性
│   ├── ColorProperty           # 颜色属性
│   ├── IconDataProperty        # 图标数据属性
│   ├── BoxConstraintsProperty  # 约束属性
│   ├── SizeProperty            # 尺寸属性
│   ├── DiagnosticLevel         # 诊断级别 (off/hint/info/warning/error)
│   ├── DiagnosticsTreeStyle    # 诊断树样式
│   ├── TextTreeConfiguration   # 文本树配置
│   ├── FlutterError            # Flutter 框架错误
│   └── FlutterError.reportError()  # 报告错误
│
├── 集合与工具
│   ├── listEquals()            # 列表深度相等
│   ├── setEquals()             # 集合深度相等
│   ├── mapEquals()             # 映射深度相等
│   ├── binarySearch<T>()       # 二分查找
│   ├── mergeSort<T>()          # 归并排序 (稳定排序)
│   ├── shortHash()             # 短哈希码
│   ├── Object.hash()           # 对象哈希 (静态方法)
│   ├── hashValues()            # 多值哈希 (已弃用)
│   ├── hashList()              # 列表哈希 (已弃用)
│   ├── clampDouble()           # 双精度钳制
│   ├── lerpDouble()            # 双精度线性插值
...
├── 元数据注解
│   ├── @immutable              # 标记不可变类
│   ├── @mustCallSuper          # 必须调用父类方法
│   ├── @protected              # 受保护成员 (仅子类/库内)
│   ├── @visibleForTesting      # 仅测试可见
│   ├── @visibleForOverriding   # 仅重写可见
│   ├── @experimental           # 实验性功能
│   ├── @optionalTypeArgs       # 可选类型参数
│   ├── @nonVirtual             # 非虚方法 (禁止重写)
│   ├── @factory               # 工厂构造函数
│   ├── @isTest                 # 标记测试函数
│   ├── @isTestGroup            # 标记测试组
│   └── @pragma()               # 编译器提示
...
│   ├── kReleaseMode             # Release 模式常量
│   ├── kDebugMode               # Debug 模式常量
│   ├── kProfileMode             # Profile 模式常量
│   ├── kIsWeb                   # 是否 Web 平台
│   ├── kIsMacOS                 # 是否 macOS
│   ├── kIsWindows               # 是否 Windows
│   ├── kIsLinux                 # 是否 Linux
│   ├── kIsAndroid               # 是否 Android
│   ├── kIsIOS                   # 是否 iOS
│   ├── kIsFuchsia               # 是否 Fuchsia
│   └── kFloatingActionButtonMargin  # FAB 边距常量
│
├── 异步与计算
│   ├── SynchronousFuture<T>     # 同步 Future (立即完成)
│   ├── compute<Q, R>()          # 在 Isolate 中执行计算
│   ├── computeMethod()          # 计算方法 (内部)
│   ├── Timeline                 # 性能时间线 (来自 dart:developer)
│   ├── TimelineTask             # 时间线任务
│   ├── Flow                     # 流程事件
│   └── debugInstrumentation()   # 调试性能工具
│
├── 错误处理
│   ├── FlutterError             # 框架错误基类
│   ├── FlutterErrorDetails      # 错误详情
│   ├── ErrorDescription         # 错误描述 (诊断节点)
│   ├── ErrorHint                # 错误提示
│   ├── ErrorSummary             # 错误摘要
│   ├── ErrorSpacer              # 错误间距
│   ├── FlutterExceptionHandler  # 异常处理器类型
│   ├── InformationCollector     # 信息收集器
│   ├── StackFilter              # 堆栈过滤器
│   ├── PartialStackFrame        # 部分堆栈帧
│   ├── RepetitiveStackFrameFilter  # 重复帧过滤器
│   └── debugStackTraceFlutterError()  # 调试堆栈
│
├── 内存与对象
│   ├── ChangeNotifier.debugAssertNotDisposed()  # 调试断言未释放
│   ├── MemoryAllocations        # 内存分配跟踪 (实验性)
│   ├── WeakReference<T>         # 弱引用 (来自 dart:core)
│   ├── Finalizer<T>             # 终结器 (来自 dart:core)
│   └── objectRuntimeType()      # 获取对象运行时类型字符串
│
└── 其他工具
    ├── describeEnum()           # 描述枚举值
    ├── describeIdentity()       # 描述对象身份
    ├── shortHash()              # 计算短哈希
