...
提供帧调度、任务优先级管理和 Ticker 支持，控制何时执行渲染任务和空闲任务，负责管理应用的帧生命周期、协调动画同步、处理 VSync 信号以及提供性能监控能力。

### 1.1 核心能力

- 帧调度（SchedulerBinding、scheduleFrame、scheduleWarmUpFrame）
- 任务优先级（Priority、idleCallback、postTask）
- Ticker 机制（Ticker、TickerProvider、TickerFuture、TickerCanceled）
- 帧回调（FrameCallback、persistentFrameCallbacks、postFrameCallbacks）
- 时间管理（timeDilation、currentFrameTimeStamp、currentSystemFrameTimeStamp）
- 性能调优（LifecycleState、schedulingStrategy）
...
│
├── 调度绑定
│   ├── SchedulerBinding          # 调度绑定混入
│   ├── SchedulerBinding.instance # 单例访问
│   ├── SchedulerBindingObserver  # 调度生命周期观察者接口
│   └── SchedulerPhase            # 调度阶段枚举
│
├── 帧管理
│   ├── FrameCallback             # 帧回调函数类型
│   ├── scheduleFrame()           # 请求新帧
│   ├── scheduleWarmUpFrame()     # 预热帧 (启动时使用)
│   ├── addPersistentFrameCallback()  # 添加持久帧回调
│   ├── addPostFrameCallback()    # 添加帧后回调
│   ├── ensureVisualUpdate()      # 确保视觉更新
│   ├── hasScheduledFrame         # 是否有已调度的帧
│   ├── currentFrameTimeStamp     # 当前帧时间戳
│   ├── currentSystemFrameTimeStamp  # 系统帧时间戳
│   ├── lastFrameTimeStamp        # 上一帧时间戳
│   ├── framesEnabled             # 帧是否启用
│   └── platformDispatcher        # 平台调度器访问
│
├── 任务调度
│   ├── Priority                  # 任务优先级类
│   ├── TaskPriority              # 任务优先级枚举
│   ├── kTouchPriority            # 触摸交互优先级
│   ├── kAnimationPriority        # 动画优先级
│   ├── kIdlePriority             # 空闲优先级
│   ├── kMaxPriority              # 最大优先级
│   ├── SchedulingStrategy        # 调度策略函数类型
│   ├── defaultSchedulingStrategy # 默认调度策略
│   ├── postTask()                # 投递任务 (内部)
│   ├── scheduleTask<T>()         # 调度任务
│   └── debugPrintBeginFrameBanner  # 调试打印帧开始标记
│
├── Ticker 机制
│   ├── Ticker                    # 动画计时器
│   ├── TickerProvider            # Ticker 提供者接口
│   ├── TickerFuture              # Ticker 完成的 Future
│   ├── TickerCanceled            # Ticker 取消异常
│   ├── SingleTickerProviderStateMixin  # 单 Ticker State 混入
│   ├── TickerProviderStateMixin  # 多 Ticker State 混入
│   ├── TickerMode                # Ticker 开关 (控制动画启用)
│   ├── TickerMode.of()           # 获取 Ticker 模式
│   ├── TickerProviderScope       # Ticker 提供者作用域 ( InheritedWidget )
│   └── debugDisableTickerMode    # 禁用 Ticker 模式调试标志
│
├── 时间控制
│   ├── timeDilation              # 时间膨胀系数 (慢动作调试)
│   ├── elapsedDuration           # 经过时间
│   ├── currentFrameDuration      # 当前帧持续时间
│   ├── currentSystemTime         # 当前系统时间
│   └── timeDilationOverride      # 时间膨胀覆盖 (调试)
│
└── 应用生命周期
    ├── AppLifecycleState         # 应用生命周期状态枚举
    ├── AppLifecycleState.resumed # 前台恢复状态
    ├── AppLifecycleState.inactive  # 非活动状态
    ├── AppLifecycleState.paused  # 后台暂停状态
    ├── AppLifecycleState.detached  # 分离状态
    ├── AppLifecycleState.hidden  # 隐藏状态 (iOS 14+)
    └── handleAppLifecycleStateChanged()  # 处理生命周期变化
```
