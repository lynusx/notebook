...
提供完整的异步编程支持，扩展了 dart:core 中的 Future 和 Stream 功能。

### 1.1 核心能力

- 异步任务调度（Future、Completer）
- 流处理与转换（Stream、StreamController、StreamTransformer）
- 事件循环与执行上下文（Zone）
- 定时器与延时执行（Timer）
- 异步工具类（StreamSubscription、AsyncError）
...
├── Future 体系
│   ├── Future<T>                 # 异步计算结果
│   ├── FutureOr<T>               # Future<T> 或 T 联合类型
│   └── Completer<T>              # Future 手动完成控制器
│
├── Stream 体系
│   ├── Stream<T>                 # 异步数据流基类
│   ├── StreamController<T>       # 单订阅流控制器
│   ├── SynchronousStreamController<T>  # 同步事件发射控制器
│   ├── MultiStreamController<T>  # 多监听器流控制器 (Stream.multi)
│   ├── StreamSink<T>             # 流数据接收器接口
│   ├── EventSink<T>              # 事件接收器 (含错误处理)
│   ├── StreamSubscription<T>     # 流订阅句柄 (控制监听)
│   ├── StreamTransformer<T, S>   # 流转换器 (map/filter 等)
│   ├── StreamIterator<T>         # await for 循环迭代器
│   └── BroadcastStreamController<T>    # 广播流控制器
│
├── 事件循环与区域
│   ├── Zone                      # 异步执行区域/环境隔离
│   ├── ZoneSpecification         # 区域行为配置规范
│   ├── ZoneDelegate              # 父区域委托句柄
│   ├── runZoned                  # 在指定 Zone 中运行代码
│   └── runZonedGuarded           # 带错误捕获的 Zone 运行
│
├── 定时器
│   └── Timer                     # 单次/周期定时器
│
└── 异步工具
    ├── AsyncError                # 异步错误及其堆栈封装
    ├── DeferredLoadException     # 延迟库加载失败异常
    ├── TimeoutException          # 异步操作超时异常
    └── unawaited                 # 标记不等待 Future (抑制警告)
```
