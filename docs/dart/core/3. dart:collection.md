# dart:collection -- 拓展集合库

提供 dart:core 集合之外的专门集合实现和集合工具函数。

### 1.1 核心能力

- 专用集合实现（LinkedList、Queue、HashMap、LinkedHashMap、HashSet、LinkedHashSet、SplayTreeMap、SplayTreeSet）
- 集合包装器（UnmodifiableListView、UnmodifiableMapView 等不可变视图）
- 集合工具函数（分组、分割、合并等）
...
├── 队列与链表
│   ├── Queue<E>                          # 双端队列接口
│   ├── ListQueue<E>                      # 基于 List 的循环队列实现
│   ├── DoubleLinkedQueue<E>              # 双向链表队列实现
│   ├── DoubleLinkedQueueEntry<E>         # 双向链表队列条目节点
│   ├── LinkedList<E extends LinkedListEntry<E>>  # 强类型双向链表
│   └── LinkedListEntry<E>                # 链表条目基类 (混入/扩展用)
│
├── 优先队列
│   └── PriorityQueue<E>                  # 基于堆的优先队列接口
│       └── HeapPriorityQueue<E>          # 二叉堆优先队列实现
│
├── 映射实现
│   ├── HashMap<K, V>                     # 哈希表 Map (无序)
│   ├── LinkedHashMap<K, V>               # 链表哈希 Map (插入顺序)
│   ├── SplayTreeMap<K, V>                # 伸展树 Map (有序，自平衡)
│   ├── CanonicalizedMap<C, K, V>         # 规范化键 Map (自定义等价性)
│   ├── MapView<K, V>                     # Map 视图包装基类
│   ├── UnmodifiableMapView<K, V>         # 不可变 Map 视图
│   ├── MapBase<K, V>                     # Map 完整实现基类
│   └── MapMixin<K, V>                    # Map 最小实现混入
│
├── 集合实现
│   ├── HashSet<E>                        # 哈希表 Set (无序)
│   ├── LinkedHashSet<E>                  # 链表哈希 Set (插入顺序)
│   ├── SplayTreeSet<E>                   # 伸展树 Set (有序，自平衡)
│   ├── SetView<E>                        # Set 视图包装基类
│   ├── UnmodifiableSetView<E>            # 不可变 Set 视图
│   ├── SetBase<E>                        # Set 完整实现基类
│   └── SetMixin<E>                       # Set 最小实现混入
│
├── 列表视图与包装
│   ├── ListBase<E>                       # List 完整实现基类
│   ├── ListMixin<E>                      # List 最小实现混入
│   └── UnmodifiableListView<E>           # 不可变列表视图
│
├── 可迭代对象工具
│   ├── IterableBase<E>                   # Iterable 完整实现基类
│   ├── IterableMixin<E>                  # Iterable 最小实现混入
│   ├── MapKeySet<K>                      # Map 键的 Set 视图
│   └── MapValueSet<K, V>                 # Map 值的 Set 视图 (去重)
│
└── 其他工具
    ├── ComparableExtension               # Comparable 扩展方法 (<, > 等)
    └── _                                 # 内部实现类 (如 _CompactLinkedHashMap)
```
