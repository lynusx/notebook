# dart:developer -- 开发者工具库

提供了与 Dart 开发者工具（如 Dart DevTools、调试器、检查器）集成的功能，仅在 JIT 编译模式和开发 JavaScript 编译器下可用，AOT 发布模式（Flutter 发布版）中功能受限。它允许开发者在代码中设置断点、记录日志、发送自定义事件、注册服务协议扩展，以及向性能时间线添加信息。

### 1.1 核心能力

- 调试输出（log、debugger）
- 时间线与性能分析（Timeline、TimelineTask、Flow）
- 代码内省（inspect）
- 扩展性支持（registerExtension、postEvent）
- 开发工具服务（Service、ServiceProtocolInfo）
...
│
├── 调试工具
│   ├── log()                   # 记录日志到 Dart DevTools 控制台
│   ├── debugger()              # 触发调试器断点 (when: 条件断点)
│   └── inspect()               # 在调试器中检查对象 (返回原对象)
│
├── 性能分析 (Timeline)
│   ├── Timeline                # 时间线事件记录静态类
│   │   ├── startSync()         # 开始同步事件
...
│   │   ├── step()              # 流程步骤
│   │   └── end()               # 结束流程
│   └── TimelineSyncFunction    # 同步函数时间线包装
│
├── 扩展与服务
│   ├── registerExtension()     # 注册 VM 服务扩展处理器
│   ├── Service                 # VM 服务协议访问
│   │   ├── getInfo()           # 获取服务协议信息
│   │   ├── controlWebServer()  # 控制 Web 服务器
│   │   └── reloadSources()     # 热重载源码
│   ├── ServiceProtocolInfo     # 服务协议连接信息
│   ├── postEvent()             # 发布自定义扩展事件
│   ├── ExtensionEvent          # 扩展事件数据类
│   └── ServiceExtensionResponse  # 服务扩展响应封装
│       ├── result              # 成功结果 JSON
│       └── error               # 错误信息
│
└── 检查器与对象 ID
    ├── getInspectorName()      # 获取对象检查器显示名称
    └── getObjectId()           # 获取对象在检查器中的唯一 ID
```
