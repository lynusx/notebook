...
提供并发编程模型，通过 Isolate（隔离区）实现并发编程，类似于线程但不共享内存（每个 Isolate 拥有独立的内存空间和事件循环），通过消息传递通信，从而避免了传统多线程编程中的数据竞争和死锁问题。

### 1.1 核心能力

- Isolate 创建与管理（Isolate、Isolate.spawn、Isolate.exit）
- 端口通信（SendPort、ReceivePort、RawReceivePort）
- 消息传递（Capability、TransferableTypedData、CrossingIsolate）
- 隔离区配置（IsolateNameServer、Isolate.spawnUri）
...
├── Isolate 管理
│   ├── Isolate                   # 隔离区实例类
│   ├── Isolate.spawn()           # 创建新隔离区 (函数 + 参数)
│   ├── Isolate.spawnUri()        # 从 URI 加载代码创建隔离区
│   ├── Isolate.exit()            # 退出当前隔离区 (可携带结果)
│   ├── Isolate.current           # 当前隔离区引用 (getter)
│   ├── IsolateNameServer         # 跨隔离区命名服务 (端口注册/查找)
│   ├── Isolate.pause()           # 暂停隔离区执行
│   ├── Isolate.resume()          # 恢复隔离区执行
│   ├── Isolate.kill()            # 终止隔离区
│   ├── Isolate.setErrorsFatal()  # 设置错误是否致命
│   ├── Isolate.addErrorListener()    # 添加错误监听器
│   ├── Isolate.removeErrorListener() # 移除错误监听器
│   ├── Isolate.addOnExitListener()   # 添加退出监听器
│   ├── Isolate.removeOnExitListener()# 移除退出监听器
│   ├── Isolate.pauseCapability       # 暂停能力令牌
│   └── Isolate.terminateCapability   # 终止能力令牌
│
├── 端口通信
│   ├── SendPort                  # 发送端口 (跨隔离区发送消息)
│   ├── ReceivePort               # 接收端口 (Stream 式消息接收)
│   ├── RawReceivePort            # 原始接收端口 (回调式消息接收)
│   └── Capability                # 能力令牌 (权限验证)
│
├── 数据传输
│   ├── TransferableTypedData     # 可零拷贝传输的类型化数据
│   └── SendPort.send()           # 发送消息方法 (支持多种数据类型)
│
└── 错误与调试
    ├── RemoteError               # 远程隔离区错误封装
    ├── IsolateSpawnException     # 隔离区创建失败异常
    └── debugName                 # 隔离区调试名称标识
```
