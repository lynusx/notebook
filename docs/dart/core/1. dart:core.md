...
提供 Dart语言最基本的内置类型、集合和基础功能，会自动导入每个 Dart程序中，无需显式使用 `import` 语句。

### 1.1 核心能力

- 基本数据类型（int、double、bool、String、Null）
- 集合框架（List、Set、Map、Iterable）
- 时间与日期（DateTime、Duration、Stopwatch）
- 正则表达式与模式匹配（RegExp、Pattern、Match）
- 异步基础（Future、Stream，已从 dart:core 导出，无需额外导入 dart:async 即可使用类型）
- 错误处理（Error、Exception、AssertionError）
- 反射与元数据（Deprecated、pragma）
- 内存管理（WeakReference、Finalizer、Expando）
- 函数与运行时表示（Function、Invocation、Type、Symbol）
- 现代 Dart 特性（Record、BigInt）
...
│   ├── Pattern             # 字符串匹配模式接口
│   ├── RegExp              # 正则表达式
│   ├── RegExpMatch         # 正则匹配结果 (含组捕获)
│   ├── Match               # 通用匹配结果接口
│   ├── Runes               # Unicode 码点序列 (UTF-32)
│   ├── RuneIterator        # 码点迭代器
│   └── StringBuffer        # 可变字符串构建器
│
├── 错误与异常
│   ├── Error               # 程序错误基类 (不可恢复)
│   ├── Exception           # 异常基类 (可恢复)
│   ├── AssertionError      # 断言失败错误
│   ├── RangeError          # 索引/范围越界错误
│   ├── NoSuchMethodError   # 方法不存在错误
│   ├── StackTrace          # 调用堆栈跟踪
│   └── Sink<T>             # 数据接收器 (常用于错误处理)
│
├── 内存管理与终结
│   ├── WeakReference<T>    # 弱引用 (不阻止 GC)
│   ├── Expando<T>          # 动态属性扩展
│   └── Finalizer<T>        # 对象终结器 (析构回调)
│
├── 函数与调用
│   ├── Function            # 函数类型基类
│   └── Invocation          # 成员调用表示 (方法名+参数)
│
├── URI 与资源定位
│   ├── Uri                 # URI/URL 解析与构建
│   └── UriData             # data: URI 结构访问
│
└── 元数据注解
    ├── Deprecated          # 弃用标记 (@Deprecated)
    └── pragma              # 编译器提示 (@pragma)
```
